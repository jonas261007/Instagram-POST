...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const avatarUrls = Object.keys(avatars); // Lista de URLs de avatares
    let currentAvatarUrl = ''; // Variável para armazenar o avatar atual
    function getAvatarName(avatarUrl) {
        return avatars[avatarUrl];
    }
    // Seleciona um avatar aleatório para o botão "Adicionar comentário"
    function setRandomAvatarForButton() {
        currentAvatarUrl = avatarUrls[Math.floor(Math.random() * avatarUrls.length)];
        const buttonAvatarImg = document.getElementById('add-comment-avatar');
        if (buttonAvatarImg) {
    ...
    if (!commentInput)
        console.error('Elemento com ID "comment-input" não encontrado.');
    if (!addCommentBtn)
        console.error('Elemento com ID "add-comment-btn" não encontrado.');
    if (!commentsList)
        console.error('Elemento com ID "comments-list" não encontrado.');
    function addComment() {
